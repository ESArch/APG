...
x3 = np.random.normal(mu[2], sigma[2], M * pi_true[2])
xs = np.concatenate((x1, x2, x3))


# Samples probability
ys = mlab.normpdf(xs, mu[0], sigma[0]) * pi_true[0] + mlab.normpdf(xs, mu[1], sigma[1]) * pi_true[1] + mlab.normpdf(xs, mu[2], sigma[2]) * pi_true[2]
...
plt.plot( x_aux, mlab.normpdf(x_aux,mu[2], sigma[2]) * pi_true[2], color="r", linestyle="-", linewidth=2)
...
pi_guess = [0.2, 0.7, 0.1] # Initial values of the parameters to stimate
...
while True:
    ...
    for k in range(K):
        for m in range(M):
            ...
            # denominator = 0
            # for k2 in range(K):
            #     denominator += pi_guess[k2] * norm(mu[k2], sigma[k2]).pdf(xs[m])
            #
            # zs[k][m] =  zs[k][m] / denominator
