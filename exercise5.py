...
for i in range(K):
    plt.plot(x_aux, mlab.normpdf(x_aux, mu[i], sigma_guess[i]) * pi_guess[i], color="black", linestyle="--", linewidth=1)
...
while True:
    ...
    sigma_guess = np.zeros(K)
    for k in range(K):
        for m in range(M):
            aux = xs[m] - mu[k]
            sigma_guess[k] += zs[k][m] * aux * aux
        sigma_guess[k] /= sum(zs[k])
        sigma_guess[k] = np.sqrt(sigma_guess[k])




    print(pi_guess, sigma_guess)



    # Draw the distributions obtained in the current iteration
    for i in range(K):
        guess_label = plt.plot(x_aux, mlab.normpdf(x_aux, mu[i], sigma_guess[i]) * pi_guess[i], color="black", linestyle="--", linewidth=1, label = "EM")
...
for i in range(K):
    final_label = plt.plot(x_aux, mlab.normpdf(x_aux, mu[i], sigma_guess[i]) * pi_guess[i], color="black", linestyle="--", linewidth=3, label="final")
